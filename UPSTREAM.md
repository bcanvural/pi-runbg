...
|---|---|---|---|
| 1 | Keep pi's built-in `bash` by default; `--replace-builtin-bash` opts into upstream's removal (upstream default removes `bash`, flag `--keep-builtin-bash` to keep) | Templates that don't know the session tools must keep a shell; user-side `bash` guards must not be silently bypassed (§7.1) | planned |
