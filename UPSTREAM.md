...
- **Contract:** tool names (`exec_command`, `write_stdin`, `set_on_exit`,
  `kill_session`, `list_sessions`) stay verbatim so prompt text remains
  portable and upstream diffs stay cherry-pickable. Schemas and constants
  hold upstream's shape except where a divergence below says otherwise
  (additive `on_output`, relaxed `set_on_exit.on_exit`,
  `MAX_YIELD_TIME_MS` 30 s → 290 s, `PREVIEW_LINES` 5 → 10).
...
|---|---|---|---|
| 1 | Remove pi's built-in `bash` by default while runbg is enabled (matching upstream's default); keep it via the persisted `/runbg replace-bash off` setting; `--replace-builtin-bash` forces removal for one invocation (upstream's `--keep-builtin-bash` flag is not registered). The flag can only force removal *on* — pi cannot distinguish an explicit `=false` from an absent boolean flag — so the setting is the only way to turn it back off. Removal/restore applied at session start and on every `/runbg` write, gated on `enabled` (a dormant runbg never leaves a prompt shell-less), with a latch so runbg only restores a `bash` it removed itself (and says so when a `/reload` cleared that record); `bash` is never traded away unless an `exec_command` is actually active. Startup warning when the upstream package is installed alongside — best-effort and load-order-dependent (if runbg's registration won, the upstream package is invisible to the registry and no warning fires). | Codex-parity removal default with fork guardrails: dormant runbg never removes, opt-out is persisted and reversible (§7.1) | landed |
...
| 10 | **Queued human input ends an attached wait** (`exec_command`'s attach, `write_stdin` polls AND input writes), reported as `wait_status: "yielded_for_user_message"`. Merged into the *preempt* signal, so the wait drains buffered output first and only then stops; the process is untouched. Disabled by `/runbg steer off`. Two constraints are worth knowing. (a) `hasPendingMessages()` counts steering AND follow-up messages together with no way to tell them apart, and pi drains them at different times — steering between tool batches, follow-ups only after the turn ends — so a follow-up would otherwise make every wait yield for the rest of the turn. Yielding is therefore deliberately UNBOUNDED, after three bounds (once-per-episode latch, start-time rule, per-episode budget) were each measured worse than none: pi runs a tool batch in PARALLEL, so any rule that can deny a wait its yield leaves a sibling in a full-length wait, the batch cannot end, and the message that caused the yield is never delivered — the feature failing at its only job. A follow-up leaving every wait to yield instantly is survivable and arguably right, since it hurries the agent toward the turn end, which is what that queue asks for. The one guard kept is a 2 s floor: waits too short for a human to be acting on are left alone, which also removes a false alarm on routine sub-second attaches. (b) `yield_until` absolute waits are deliberately NOT steer-aware: they are explicitly heartbeat-free, and the human has already opted into a long attached wait. Not in upstream or codex | pi delivers steering only after every tool call in a batch finishes, so a long attached wait holds the human's message hostage — and with plain `bash` the only escape is Esc, which kills the command. Ending the wait is safe here precisely because the session owns the process. Modelled on oh-my-pi, which skips a pending tool call outright when a message is queued (*"Do not count this skipped result as completed work… retry the skipped tool if it is still needed"*). This is a workaround for a host-level gap: the complete fix belongs in pi, which could defer pending tool calls batch-wide for every tool, not just these five | landed |
...
| 12 | **`set_on_exit`'s `on_exit` parameter is relaxed from required to optional** (omitted = that arm is left unchanged), and `exec_command` / `set_on_exit` gain a new optional `on_output` parameter (`{pattern, case_sensitive?}` arms a readiness wake on `exec_command`; on `set_on_exit` an object re-arms/replaces it with a fresh generation, `null` disarms the match arm, omitted = unchanged). Match wakes arrive with a new `customType: "runbg-matched"` (exit wakes keep `"runbg-completed"`) | Per-arm wake-policy editing must be able to touch only the arm being changed: a required `on_exit` would force a match-only re-arm to pass a field it is not touching, and record-level suppression would kill the wrong arm. The additions are optional params — additive shape; the relaxation of `on_exit` is the loud part and is deliberate (design §7) | landed (IV-0004) |
| 13 | **Held-open session diagnosis — port of upstream 0.9.1 (`8493da5`) with deliberate extensions** (IV-0006): a session whose shell has exited but whose output pipe is held by background processes reports `[still running]` with a `note` header line (platform-aware remedy text) and `(shell exited, pipe held)` markers. Extensions beyond upstream: (a) `list_sessions.shell_exited` field + `[shell exited]` text/TUI markers — upstream has none; (b) platform-specific remedy text (Windows tasklist/taskkill-or-wait; POSIX setsid escape qualifier) vs upstream's platform-neutral constant; (c) kill-failed results append the platform note when `!killed && !session.hasExited && session.shellExited`; (d) TUI result-row note slot; (e) the note covers all six of runbg's still-running result sites (upstream: three) — a consequence of #7/#8. The note also documents the wake contract (an armed `on_exit` wake stays pending until the last pipe holder exits). The wrapper-level derivation is mechanism, not observable behavior — IV-internal only | Diagnosis is state-driven and truthful: upstream's note overpromises `kill_session` (Windows taskkill live-root; POSIX setsid escape), and runbg's `list_sessions` is already a richer audit surface; the model must never be told to retry a structurally futile kill | landed (IV-0006) |
